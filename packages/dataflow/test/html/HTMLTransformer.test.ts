...
return {b};
    ...
    test("single expressions will create a placeholder display slot", async () => {
        ...
        expect(result).toBe(`<body><slot name="_display_4vhz4q"></slot><script type="module">${scriptTemplate(`renderer.register("vge10p",[],["a"],() => {
const a = 1;
return {a};
});
renderer.register("_display_4vhz4q",["a"],[],(a) => \`Some text \${a}\`);`)}</script></body>`);
    });
    ...
    test("if the javascript is invalid, report the error in the slot", async () => {
        ...
        expect(result).toBe(`<body><slot name="_display_00001p"></slot><script type="module">${scriptTemplate(`renderer.register("_display_00001p",[],[],() => "Unexpected token (1:0)");`)}</script></body>`);
    });
...
const name = iterator(notify => input.addEventListener('input', ev => {notify(ev.data)}), input.value);
        ...
        expect(result).toBe(`<body><script type="module">${scriptTemplate(`renderer.register("uf397b",["jsx"],["input","name","iterator"],async(jsx) => {
...
return {input,name,iterator};
    ...
    test("data-echo inserts escaped code block after output", async () => {
        ...
        expect(result).toBe(`<body><pre><code class="language-javascript">1 + 2</code></pre><slot name="_display_0rj9ce"></slot><script type="module">${scriptTemplate(`renderer.register("_display_0rj9ce",[],[],() => 1 + 2);`)}</script></body>`);
    });
